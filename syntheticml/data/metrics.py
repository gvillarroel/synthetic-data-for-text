...
class Metrics:
    ...
    def privacy(self, df_base: pd.DataFrame, df_target: pd.DataFrame):
        ...
        X_real = self.mms.transform(df_base.loc[:, self.columns_number])
        X_fake = self.mms.transform(df_target.loc[:, self.columns_number])
    ...
    def get_scores(self, fake_data: dict, report_folder) -> tuple[pd.DataFrame, dict]:
        scores = []
        reports = {}
        score_path = f"{report_folder}/scores.parquet"
        for model_name, df_fake in fake_data.items():
        ...
        if not os.path.exists(score_path):
            ...
            pd_scores.to_parquet(score_path)
        else:
            pd_scores  = pd.read_parquet(score_path)
